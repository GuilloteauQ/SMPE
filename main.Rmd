...
for (y in df$year) {
  year_CO2 <- subset(df, year == y, select = CO2)
  tmp <- df[df$year == y,]
  df[df$year == y,]$CO2_year <- df[df$year == y,]$CO2 - tmp$CO2[1]
}
...
data2019$prediction <- predict(reg_poly, df[df$year == 2019,])
...
```{r}
reg_sin <- lm(data = derivative_data, CO2_year ~ sin(2*pi/366*day_of_year) + cos(2*pi/366*day_of_year) + poly(day_of_year, 2))
summary(reg_sin)
